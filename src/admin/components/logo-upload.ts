...
export class LogoUpload extends LitElement {
    ...
    private processFile(file: File) {
        ...
        // Validate file type (SVG not supported due to XSS risk)
        const validTypes = ['image/png', 'image/jpeg'];
        if (!validTypes.includes(file.type)) {
            this.error = 'Invalid file type. Please upload PNG or JPG.';
            return;
        }
        ...
        return html`
            ...
            >
                ...
                ` : html`
                    ...
                    <p class="upload-hint">PNG or JPG (max 2MB, recommended 200x80px)</p>
                `}
            </div>
